...
for n in range(n_samples):
    ...
    for problem in np.arange(1, 3):
        ...
        # ESN cell
        esn = etnn.BDESN(
            input_dim=tools.settings.lang_models_dim[args.lang],
    ...
    # Sample f1 score
    this_f1_score = np.average(single_sample_average)

    # Best score?
    if this_f1_score > best_acc:
        print(u"New best model with {}".format(this_f1_score))
        torch.save(w, open("./W/" + args.lang + ".pth", 'wb'))
        best_model = w
        best_acc = this_f1_score
    # end if
# end for
